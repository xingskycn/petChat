...
@synthesize shareBtn,shareCountLabel,shareIamgeView;
@synthesize playBtn,playImageView;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier AndData:(NSMutableArray *)dataArray AndString:(NSString *)signString
{
    ...
    if (self) {
        ...
        //分享数目label
        shareCountLabel = [[UILabel alloc]init];
        
        //分享图标
        shareIamgeView = [[UIImageView alloc]init];
        ...
        //分享
        shareIamgeView.backgroundColor = [UIColor clearColor];
        shareIamgeView.image = [UIHelper imageName:@"vedio_share"];
        shareCountLabel.textColor =  [UIHelper colorWithHexString:@"#d6d6d6"];
        shareCountLabel.font = [UIFont systemFontOfSize:11.0f];
        ...
        [passPhotoImageView addSubview:playImageView];
        [passPhotoImageView addSubview:playBtn];
        [self addSubview:passPhotoImageViewTwo];
        [self addSubview:timeLabel];
//        [self addSubview:shareIamgeView];
...
//        [self addSubview:markButton];
        ...
        if(photosArray.count >=2)
        {
            ...
            for(int i = 0;i< h;i++)
            {
                int m = i/3;//行数,从1开始
                int n = i%3;//该行有多少个
                NSString *videoString = [photosArray objectAtIndex:i];
        ...
        else if (h == 1)
        {
            ...
            NSString *videoString = [photosArray objectAtIndex:0];
            photosImageView = [[img alloc]init];
...
- (void)passPhotoButtonIsPressed:(id)sender{
    ...
    NSLog(@"交出委托事件，当前点击的图片索引：%d",i);
}

- (void)vedioBtnClicked:(NSString *)url{
    [self.newsCellButtonClickDelegate playVedioClick:url];
}
