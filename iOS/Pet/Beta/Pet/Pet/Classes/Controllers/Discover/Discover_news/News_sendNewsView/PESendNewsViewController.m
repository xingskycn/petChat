...
#import "UIHelper.h"
@interface PESendNewsViewController ()

@end

@implementation PESendNewsViewController

@synthesize sendNewsView, sheet, sheet2, dataArray, type, newsType, videoURL,hasMp4,mp4Path,hasVedio,alertView;
@synthesize sendBtn;
...
- (void)viewDidLoad
{
    ...
    titleLabel.text=NSLocalizedString(DISCOVER_NEWS_SENDNEWS_TITLE, nil);;
    ...
    sendNewsView = [[PESendNewsView alloc]initWithFrame:CGRectMake(0, 64, 320, ScreenHeight-64)];
    sendNewsView.delegate = self;
    sendNewsView.shoutSortBtn.hidden = YES;
    newsType =[[NSString alloc] init];
    [self.view addSubview:sendNewsView];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (actionSheet ==sheet) {
        ...
        sendNewsView.shoutLabel.text =newsType;
    }
...
- (void)sendNews {
    ...
    //输入的想对朋友说些什么的内容
    //输入的想对朋友说些什么的内容
    if(sendNewsView.myTextView.text.length ==0){
        [Common showAlert:@"发送动态内容不能为空喔！"];
        return;
    }
    NSDictionary *contentInfo =@{@"content": sendNewsView.myTextView.text};
    NSDictionary *typeInfo =@{@"type": @""};
...
//        sendNewsView.tempwordslabel.text = @"120/120字";
    ...
    //会根据type判断是视频还是字典
    [[PENetWorkingManager sharedClient] sendNews:request data:dataArray completion:^(NSDictionary *results, NSError *error) {
        if (results) {
            NSLog(@"%@",results);
            sendBtn.enabled = NO;
