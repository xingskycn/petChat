...
public class HttpRequsetCallback extends AsyncTask<Object, Integer, JSONTokener>{
	...
	protected JSONTokener doInBackground(Object... params) {
		...
		try {
			...
			if(requestType==RequestType.REQUEST_UPLOAD_IMAGE){
				...
				String reqesutkey=parameters.get(0).getRequestHttpKey();
				String value=parameters.get(0).getValue();
				String reqesutkeyfilePath=parameters.get(1).getRequestHttpKey();
				String uploadFile=parameters.get(1).getValue();
				obj=httpHelper.uploadFile_Paramters(reqesutkey,value,requestUrl,reqesutkeyfilePath,uploadFile);
			}
			...
			if(!obj.equals("")){
				jsonTokener=new JSONTokener(obj);
			}
	...
	protected void onPostExecute(JSONTokener result) {
		...
		if(jsonTokener!=null){
			httpRequestInterfer.onTaskRequestSuccess(taskId, jsonTokener);
		}
		else if(bitmap!=null){
